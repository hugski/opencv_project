...
auto main() -> int {
    ...
    for (int i = 1; i <= 4; i++) {
        ...
        str << "chessboards/chess" << std::setw(1)
            << i << ".jpg";
        ...
        if (image.empty()){
            std::cout << "open image error: " << str.str() << std::endl;
            return -1;
        }
    ...
    cv::Size newSize(static_cast<int>(image.cols * 1.5),
                     static_cast<int>(image.rows * 1.5));
    cv::Mat uImage = cameraCalibrator.remap(image, newSize);

    cv::Mat cameraMatrix = cameraCalibrator.getCameraMatrix();

    std::cout << "内参矩阵: " << cameraMatrix.rows << "x" << cameraMatrix.cols
              << std::endl;
    ;
